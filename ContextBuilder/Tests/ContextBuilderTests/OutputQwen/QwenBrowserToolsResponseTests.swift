...
internal struct QwenBrowserToolsResponseTests {
    @Test(
    ...
    private func loadExpectedBrowserToolsResponseOutput() throws -> String {
        guard let url = Bundle.module.url(
            forResource: "Example-Qwen/output_qwen_browser_tools_response",
            withExtension: "txt"
        ) else {
            throw ContextBuilderTestError.resourceNotFound(
                "Example-Qwen/output_qwen_browser_tools_response.txt"
            )
        }
