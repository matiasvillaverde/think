...
@Suite("Harmony Test: Harmony Input Test Simple Convo Low Effort No Instruction")
internal struct HarmonySimpleConvoLowNoInstr {
    @Test(
        "Validates harmony_input_test_simple_convo_low_effort_no_instruction",
    ...
    func test(architecture: Architecture) async throws {
        ...
        let expectedOutput = try loadResource(
            "harmony_input_test_simple_convo_low_effort_no_instruction"
        )
