...
@Suite("Harmony Test: Harmony Output Test Does Not Drop If Ongoing Analysis")
internal struct HarmonyNoDropOngoingAnalysis {
    @Test(
        "Validates harmony_output_test_does_not_drop_if_ongoing_analysis",
    ...
    func test(architecture: Architecture) async throws {
        ...
        let expectedOutput = try loadResource(
            "harmony_output_test_does_not_drop_if_ongoing_analysis"
        )
