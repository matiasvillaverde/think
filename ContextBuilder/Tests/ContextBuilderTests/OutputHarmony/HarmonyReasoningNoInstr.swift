...
@Suite("Harmony Test: Harmony Input Test Reasoning System Message No Instruction")
internal struct HarmonyReasoningNoInstr {
    @Test(
        "Validates harmony_input_test_reasoning_system_message_no_instruction",
    ...
    func test(architecture: Architecture) async throws {
        ...
        let expectedOutput = try loadResource(
            "harmony_input_test_reasoning_system_message_no_instruction"
        )
        ...
        let messages = [
            TestHelpers.createMessageDataWithChannels(
                userInput: "What is the best place to eat candy in the world?",
                channels: []
            )
        ]
