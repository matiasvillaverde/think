...
@Suite("Gemma3 VLM Generation Tests")
struct Gemma3VLMModelTest {
    let baseTest = BaseModelTest()

    @Test("Generate text with Gemma3 VLM model and image input")
    func testGemma3VLMGeneration() async throws {
        guard let modelURL: URL = baseTest.getModelURLIfAvailable(
            resourceName: "gemma-3-4b-it-qat-3bit",
            in: Bundle.module
        ) else {
            return
        }
        ...
        let config = ProviderConfiguration(
            ...
            modelName: "gemma-3-4b-it-qat-3bit",
            compute: .small
        )
