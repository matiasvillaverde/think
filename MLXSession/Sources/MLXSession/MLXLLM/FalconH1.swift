...
private func createSSMMask(h: MLXArray, cache: MambaCache?) -> MLXArray? {
    cache?.makeMask(N: h.dim(1))
}

private func createFalconAttentionMask(h: MLXArray, cache: [KVCache]?) -> MLXArray? {
    let N = h.dim(1)
...
internal class FalconH1ModelInner: Module {
        ...
        -> MLXArray
    {
        ...
        let attnMask: MLXArray? = createFalconAttentionMask(
