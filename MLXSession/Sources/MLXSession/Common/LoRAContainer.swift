...
/// }
/// ```
internal struct LoRAConfiguration: Codable, Sendable {

    internal enum FineTuneType: String, Codable, Sendable {
        case lora
        case dora
    }

    internal struct LoRAParameters: Codable, Sendable {
    ...
    internal let numLayers: Int
    internal let fineTuneType: FineTuneType
    internal let loraParameters: LoRAParameters
...
/// - It contains `ModuleParameters` which wraps MLX arrays
/// - MLX arrays are NOT inherently Sendable but are used in a controlled way
/// - The parameters are immutable after creation
/// - Loading/unloading/fusing adapters modifies the model, not this container
///
/// Safety guarantees:
/// - Immutable after creation: Both `configuration` and `parameters` are `let` constants
/// - No shared mutable state: The container itself doesn't hold mutable state
/// - Model modification is controlled: Adapter operations modify the language model, not this container
/// - Thread-safe when used correctly: Each model should only be modified from one thread at a time
internal struct LoRAContainer: ModelAdapter, @unchecked Sendable {

    /// The configuration used to construct this adapter container.
    internal let configuration: LoRAConfiguration
    /// The parameter values for the adapter modules.
    internal let parameters: ModuleParameters
    ...
    internal static func from(directory: URL) throws -> LoRAContainer {
    ...
    internal func load(into model: LanguageModel) throws {
        guard let lora = model as? LoRAModel else {
            throw ModelAdapterError.incompatibleModelType
        }
    ...
    internal func fuse(with model: LanguageModel) throws {
        guard let lora = model as? LoRAModel else {
            throw ModelAdapterError.incompatibleModelType
        }
    ...
    internal func unload(from model: LanguageModel) {
        guard let lora = model as? LoRAModel else {
