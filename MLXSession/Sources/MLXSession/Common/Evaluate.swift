...
///
/// Safety guarantees:
/// - Immutable configuration: `temp` and `topP` are never modified after creation
...
internal struct TopPSampler: LogitSampler, @unchecked Sendable {
    ...
    internal init(temperature: Float, topP: Float) {
...
internal struct CategoricalSampler: LogitSampler, @unchecked Sendable {
    ...
    internal init(temperature: Float) {
        self.temp = MLXArray(temperature)
        self.randomState = MLXRandom.RandomState()
    }
...
internal struct RepetitionContext: LogitProcessor {
