...
internal struct Qwen3VLProcessor: UserInputProcessor {

    private let config: Qwen3VLProcessorConfiguration
    private let tokenizer: any Tokenizer

    internal init(_ config: Qwen3VLProcessorConfiguration, tokenizer: any Tokenizer) {
        self.config = config
    ...
    private func preprocess(image: CIImage, resizedSize: CGSize) -> CIImage {
        ...
        let resized = MediaProcessing.resampleBicubic(srgb, to: resizedSize)
...
internal struct Qwen3VLProcessorConfiguration: Codable, Sendable {

    internal struct Size: Codable, Sendable {
        internal let maxPixels: Int
        internal let minPixels: Int
    ...
    internal let imageMean: [CGFloat]
    internal let imageStd: [CGFloat]
    private let _minPixels: Int?
    private let _maxPixels: Int?
    internal let mergeSize: Int
    internal let patchSize: Int
    internal let temporalPatchSize: Int
    internal let imageProcessorType: String

    internal var minPixels: Int { _minPixels ?? 4 * 28 * 28 }  // 3,136
    internal var maxPixels: Int { _maxPixels ?? 16384 * 28 * 28 }  // 12,845,056

    internal var size: Size { .init(maxPixels: maxPixels, minPixels: minPixels) }
...
internal struct Qwen3VLConfiguration: Codable, Sendable {

    internal struct TextConfiguration: Codable, Sendable {
        ...
        internal var numKeyValueHeads: Int { _numKeyValueHeads ?? numAttentionHeads }
        internal let headDim: Int
        private let _ropeTheta: Double?
        internal var ropeTheta: Double { _ropeTheta ?? 1_000_000 }
        internal let maxPositionEmbeddings: Int
        private let _rmsNormEps: Double?
        internal var rmsNormEps: Double { _rmsNormEps ?? 1e-6 }
        private let _ropeScaling: RoPEScaling?
        internal var ropeScaling: RoPEScaling? { _ropeScaling }
        private let _normTopKProb: Bool?
        internal var normTopKProb: Bool { _normTopKProb ?? true }
        private let _tieWordEmbeddings: Bool?
        internal var tieWordEmbeddings: Bool { _tieWordEmbeddings ?? true }
        private let _attentionBias: Bool?
        internal var attentionBias: Bool { _attentionBias ?? false }
        private let _hiddenAct: String?
        internal var hiddenAct: String { _hiddenAct ?? "silu" }
    ...
    internal struct VisionConfiguration: Codable, Sendable {
        internal let modelType: String
        internal let depth: Int
        internal let hiddenSize: Int
        internal let intermediateSize: Int
        internal let outHiddenSize: Int
        internal let numHeads: Int
        internal let patchSize: Int
        internal let spatialMergeSize: Int
        internal let temporalPatchSize: Int
        internal let numPositionEmbeddings: Int
        private let _inChannels: Int?
        internal var inChannels: Int { _inChannels ?? 3 }
        private let _hiddenAct: String?
        internal var hiddenAct: String { _hiddenAct ?? "gelu" }
        private let _deepstackVisualIndexes: [Int]?
        internal var deepstackVisualIndexes: [Int] { _deepstackVisualIndexes ?? [] }
    ...
    internal struct RoPEScaling: Codable, Sendable {
        ...
        internal init(type: String? = nil, mropeInterleaved: Bool? = nil, mropeSection: [Int]? = nil)
        {
            self.type = type
    ...
    internal let textConfiguration: TextConfiguration
    internal let visionConfiguration: VisionConfiguration
    internal let modelType: String
    private let _ignoreIndex: Int?
    internal var ignoreIndex: Int { _ignoreIndex ?? -100 }
    private let _imageTokenId: Int?
    internal var imageTokenId: Int { _imageTokenId ?? 151_655 }
    private let _videoTokenId: Int?
    internal var videoTokenId: Int { _videoTokenId ?? 151_656 }
    private let _imageTokenIndex: Int?
    internal var imageTokenIndex: Int { _imageTokenIndex ?? imageTokenId }
    private let _videoTokenIndex: Int?
    internal var videoTokenIndex: Int { _videoTokenIndex ?? videoTokenId }
    private let _visionStartTokenId: Int?
    internal var visionStartTokenId: Int { _visionStartTokenId ?? 151_652 }
    private let _visionEndTokenId: Int?
    internal var visionEndTokenId: Int { _visionEndTokenId ?? 151_653 }
    private let _visionTokenId: Int?
    internal var visionTokenId: Int { _visionTokenId ?? 151_654 }
    private let _vocabSize: Int?
    internal var vocabSize: Int { _vocabSize ?? textConfiguration.vocabSize }
    private let _eosTokenId: [Int]?
    internal var eosTokenId: [Int]? { _eosTokenId }
...
internal final class Qwen3VL: Module, VLMModel, KVCacheDimensionProvider {

    @ModuleInfo(key: "vision_tower") private var visionModel: Qwen3VLVision.VisionModel
    @ModuleInfo(key: "language_model") private var languageModel: Qwen3VLLanguage.LanguageModel

    internal let config: Qwen3VLConfiguration

    internal init(_ config: Qwen3VLConfiguration) {
        self.config = config
    ...
    internal var vocabularySize: Int { config.vocabSize }
...
internal struct Qwen3VLMessageGenerator: MessageGenerator {
    internal init() {}
