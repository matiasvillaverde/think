...
internal struct Gemma3VLMTextConfiguration: Codable, Sendable {
    ...
    // Not included in 4B and 12B model config.json, included for 27B model
    internal var queryPreAttnScalar: Float {
    ...
    init(from decoder: Swift.Decoder) throws {
        ...
        modelType = try container.decodeIfPresent(String.self, forKey: .modelType) ?? "gemma3"
        ...
        vocabularySize = try container.decodeIfPresent(Int.self, forKey: .vocabularySize) ?? 262_208
...
internal struct Gemma3VLMVisionConfiguration: Codable, Sendable {
    let modelType: String
    let hiddenLayers: Int
    let hiddenSize: Int
    let intermediateSize: Int
...
internal struct Gemma3VLMConfiguration: Codable, Sendable {
    let textConfiguration: Gemma3VLMTextConfiguration
    let visionConfiguration: Gemma3VLMVisionConfiguration
    let modelType: String
    let mmTokensPerImage: Int
...
private class Attention: Module {
    ...
    init(config: Gemma3VLMTextConfiguration, layerIdx: Int) {
...
// MARK: - Gemma3VLMTextBackbone

private class Gemma3VLMTextBackbone: Module {
    @ModuleInfo(key: "embed_tokens") var embedTokens: Embedding
    @ModuleInfo var layers: [TransformerBlock]
    @ModuleInfo var norm: Gemma.RMSNorm

    let config: Gemma3VLMTextConfiguration

    init(_ config: Gemma3VLMTextConfiguration) {
        self.config = config
    ...
    func callAsFunction(
        ...
        let globalCache: [KVCache]? = {
            guard let cache, config.slidingWindowPattern > 0 else { return nil }
            if let global = cache[config.slidingWindowPattern - 1] {
                return [global]
            }
            return nil
        }()
        let globalMask = createAttentionMask(h: h, cache: globalCache, returnArray: true)
        let slidingWindowMask =
            if config.slidingWindowPattern > 1 {
                createSlidingWindowMask(
                    h: h,
                    cache: cache?.first ?? nil,
...
private final class Gemma3VLMTextModel: Module, KVCacheDimensionProvider {
    @ModuleInfo var model: Gemma3VLMTextBackbone
    ...
    init(_ config: Gemma3VLMTextConfiguration) {
        self.config = config
        self.model = Gemma3VLMTextBackbone(config)
        ...
        -> [String: MLXArray]
    {
        ...
        if hasQuantizedLmHead {
            ...
            let q = quantizationConfig?.asTuple ?? (64, 4)
            let mode = quantizationConfig.map(quantizationMode(for:)) ?? .affine
...
private class VisionMLP: Module, UnaryLayer {
    ...
    init(config: Gemma3VLMVisionConfiguration) {
...
private class VisionEmbeddings: Module, UnaryLayer {
    ...
    let config: Gemma3VLMVisionConfiguration
    let embedDim: Int
    let imageSize: Int
    let patchSize: Int
    let numPatches: Int
...
private final class Gemma3VLMVisionModel: Module {
...
class Gemma3VLMMultiModalProjector: Module, UnaryLayer {
    ...
    let config: Gemma3VLMConfiguration
    let patchesPerImage: Int
    let tokensPerSide: Int
    let kernelSize: Int

    init(config: Gemma3VLMConfiguration) {
        self.config = config
...
internal final class Gemma3VLMModel: Module, VLMModel, KVCacheDimensionProvider {
    @ModuleInfo(key: "vision_tower") private var visionTower: Gemma3VLMVisionModel
    @ModuleInfo(key: "language_model") private var languageModel: Gemma3VLMTextModel
    @ModuleInfo(key: "multi_modal_projector") var multiModalProjector: Gemma3VLMMultiModalProjector

    let config: Gemma3VLMConfiguration

    internal var vocabularySize: Int { config.vocabularySize }
    internal var kvHeads: [Int] { languageModel.kvHeads }
    ...
    init(_ config: Gemma3VLMConfiguration) {
        self.config = config

        self._visionTower.wrappedValue = Gemma3VLMVisionModel(config: config.visionConfiguration)
        self._languageModel.wrappedValue = Gemma3VLMTextModel(config.textConfiguration)
        self._multiModalProjector.wrappedValue = Gemma3VLMMultiModalProjector(config: config)
    }
    ...
    internal func prepare(_ input: LMInput, cache: [any KVCache], windowSize: Int?) throws
        -> PrepareResult
    {
    ...
    internal func callAsFunction(_ inputs: MLXArray, cache: [any KVCache]?) -> MLXArray {
...
internal struct Gemma3VLMProcessor: UserInputProcessor {
    private let config: Gemma3VLMProcessorConfiguration
    private let tokenizer: any Tokenizer

    init(_ config: Gemma3VLMProcessorConfiguration, tokenizer: any Tokenizer) {
        self.config = config
        self.tokenizer = tokenizer
    }

    internal func preprocess(images: [CIImage], processing: UserInput.Processing?) throws -> (
        MLXArray, THW
    ) {
...
internal struct Gemma3VLMProcessorConfiguration: Codable, Sendable {
    ...
    // Optional fields
    let doConvertRgb: Bool?
    let doPanAndScan: Bool?
    let panAndScanMaxNumCrops: Int?
    let panAndScanMinCropSize: Int?
    let panAndScanMinRatioToActivate: Float?

    // Image token identifier from model configuration
    let imageTokenId: Int = 262144

    internal struct ImageSize: Codable, Sendable {
        let height: Int
        let width: Int
    }

    // Computed properties for convenience
    internal var imageSize: Int { size.height }

    internal var imageMeanTuple: (CGFloat, CGFloat, CGFloat) {
        (imageMean[0], imageMean[1], imageMean[2])
    }

    internal var imageStdTuple: (CGFloat, CGFloat, CGFloat) {
...
extension Gemma3VLMModel: LoRAModel {
