...
public final class DiscreteFlowScheduler: Scheduler {
    ...
    public func calculateTimestepsFromSigmas(strength: Float?) -> [Float] {
        ...
        let startStep = max(
            inferenceStepCount - Int(Float(inferenceStepCount) * strength),
            0
        )
        return Array(timestepsFromSigmas()[startStep...])
    }
    ...
    public func step(
        ...
        // Allow float timesteps in scheduler step protocol
        ...
        let prevSample: MLShapedArray<Float32> = MLShapedArray<Float>(
            unsafeUninitializedShape: modelOutput.shape
        ) { result, _ in
            modelOutput.withUnsafeShapedBufferPointer { noiseScalars, _, _ in
                sample.withUnsafeShapedBufferPointer { latentScalars, _, _ in
                    for i in 0..<result.count {
