...
struct NvRandomSource: RandomSource {
  ...
  private static let PHILOX_M4_32: (UInt32, UInt32) = (0xD251_1F53, 0xCD9E_8D57)
  private static let PHILOX_W_32: (UInt32, UInt32) = (0x9E37_79B9, 0xBB67_AE85)
  ...
  private static func philox4_32(counter: inout [[UInt32]], key: inout [[UInt32]], rounds: Int = 10) {
    for _ in 0..<(rounds - 1) {
  ...
  private func boxMuller(_ counter1: [UInt32], _ counter2: [UInt32], mean: Double, stdev: Double) -> [Double] {
    ...
    zip(counter1, counter2).map {
      let u: Double = Double($0) / 4_294_967_296.0 + (1.0 / 8_589_934_592.0)
      let v: Double = Double($1) * (.pi / 2_147_483_648.0) + (.pi / 4_294_967_296.0)
