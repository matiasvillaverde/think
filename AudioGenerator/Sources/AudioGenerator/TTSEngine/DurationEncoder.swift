...
internal class DurationEncoder {
    var lstms: [Module] = []
    ...
    func callAsFunction(_ x: MLXArray, style: MLXArray, textLengths _: MLXArray, m: MLXArray) -> MLXArray {
        var x: MLXArray = x.transposed(2, 0, 1)
        let s: MLXArray = MLX.broadcast(style, to: [x.shape[0], x.shape[1], style.shape[style.shape.count - 1]])
        ...
        for block in lstms {
            if let adaLayerNorm = block as? AdaLayerNorm {
                ...
                let (lstmOutput, _): (MLXArray, ((MLXArray, MLXArray), (MLXArray, MLXArray))) = lstm(x)
                x = lstmOutput.transposed(0, 2, 1)
                let xPad: MLXArray = MLXArray.zeros([x.shape[0], x.shape[1], m.shape[m.shape.count - 1]])
