...
internal class TextEncoder {
    ...
    init(weights: [String: MLXArray], channels: Int, kernelSize: Int, depth: Int, nSymbols _: Int, actv: Module = LeakyReLU(negativeSlope: 0.2)) {
        ...
        let padding: Int = (kernelSize - 1) / 2
    ...
    func callAsFunction(_ x: MLXArray, inputLengths _: MLXArray, m: MLXArray) -> MLXArray {
        var x: MLXArray = embedding(x)
        x = x.transposed(0, 2, 1)
        let mask: MLXArray = m.expandedDimensions(axis: 1)
        ...
        let xPad: MLXArray = MLX.zeros([x.shape[0], x.shape[1], mask.shape[mask.shape.count - 1]])
