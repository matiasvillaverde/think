...
internal struct SineGenOutput {
    let sineWavesWithoutNoise: MLXArray
    let sineWavesWithNoise: MLXArray
    let uppProb: MLXArray
}
import MLXNN
import MLXRandom

internal class SineGen {
    ...
    private func _f02uv(_ f0: MLXArray) -> MLXArray {
        let arr: MLXArray = f0 .> voicedThreshold
        return arr.asType(.float32)
    }

    private func _f02sine(_ f0Values: MLXArray) -> MLXArray {
        // Ignore integer part (% 1 is there for a purpose :)
        var radValues: MLXArray = (f0Values / Float(samplingRate)) % 1

        // Random phase noise
        let randIni: MLXArray = MLXRandom.normal([f0Values.shape[0], f0Values.shape[2]])
        ...
        var phase: MLXArray = MLX.cumsum(radValues, axis: 1) * 2 * Float.pi
        phase = interpolate(
    ...
    func callAsFunction(_ f0: MLXArray) -> SineGenOutput {
        // Fundamental component
        let range: MLXArray = MLXArray(1 ... harmonicNum + 1).asType(.float32)
        let fn: MLXArray = f0 * range.reshaped([1, 1, range.shape[0]])

        // Generate sine waveforms
        let sineWaves: MLXArray = _f02sine(fn) * sineAmp

        // Generate UV signal
        let uv: MLXArray = _f02uv(f0)

        // Generate noise
        let noiseAmp: MLXArray = uv * noiseStd + (1 - uv) * sineAmp / 3
        let noise: MLXArray = noiseAmp * MLXRandom.normal(sineWaves.shape)

        let result: MLXArray = sineWaves * uv + noise
        return SineGenOutput(
            sineWavesWithoutNoise: result,
            sineWavesWithNoise: uv,
            uppProb: noise
        )
    }
