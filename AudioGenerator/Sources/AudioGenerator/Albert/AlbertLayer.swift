...
internal class AlbertLayer {
    ...
    func ffChunk(_ attentionOutput: MLXArray) -> MLXArray {
        var ffnOutputArray: MLXArray = ffn(attentionOutput)
    ...
    func callAsFunction(
        ...
        let attentionOutput: MLXArray = attention(hiddenStates, attentionMask: attentionMask)
        let ffnOutput: MLXArray = ffChunk(attentionOutput)
        return fullLayerLayerNorm(ffnOutput + attentionOutput) as MLXArray
    }
