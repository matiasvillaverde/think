...
internal class AlbertEmbeddings {
    ...
    init(weights: [String: MLXArray], config: AlbertModelArgs) {
        ...
        let layerNormWeights: MLXArray = weights["bert.embeddings.LayerNorm.weight"]!
        let layerNormBiases: MLXArray = weights["bert.embeddings.LayerNorm.bias"]!
    ...
    func callAsFunction(
        ...
        let seqLength: Int = inputIds.shape[1]
        ...
        let wordsEmbeddings: MLXArray = wordEmbeddings(inputIds)
        let positionEmbeddingsResult: MLXArray = positionEmbeddings(positionIdsUsed)
        let tokenTypeEmbeddingsResult: MLXArray = tokenTypeEmbeddings(tokenTypeIdsUsed)
        var embeddings: MLXArray = wordsEmbeddings + positionEmbeddingsResult + tokenTypeEmbeddingsResult
