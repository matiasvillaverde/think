...
internal class AdaINResBlock1 {
    ...
    init(
        ...
        for i in 0 ..< 3 {
            let dilationValue: Int = dilation[i]
            let conv: ConvWeighted = ConvWeighted(
    ...
    func callAsFunction(_ x: MLXArray, _ s: MLXArray) -> MLXArray {
        var result: MLXArray = x

        for i in 0 ..< convs1.count {
            let c1: ConvWeighted = convs1[i]
            let c2: ConvWeighted = convs2[i]
            let n1: AdaIN1d = adain1[i]
            let n2: AdaIN1d = adain2[i]
            let a1: MLXArray = alpha1[i]
            let a2: MLXArray = alpha2[i]

            var xt: MLXArray = n1(result, s: s)
...
// swiftlint:enable force_unwrapping shorthand_operator
