import Foundation
import MLX
import MLXNN

internal func computeNorm(
    x: MLXArray,
    p: Int,
    dim: [Int]? = nil,
    keepdim: Bool = false
) -> MLXArray {
    guard p == 1 || p == 2 else {
        fatalError("Only p-norms with p of 1 or 2 are supported")
    }

    let dimensions: [Int]
    if let dim {
        dimensions = dim
    } else {
        dimensions = Array(0 ..< x.ndim)
    }

    if p == 1 {
        // L1 norm
        return MLX.sum(MLX.abs(x), axes: dimensions, keepDims: keepdim)
    }
    // L2 norm
    return MLX.sqrt(MLX.sum(x * x, axes: dimensions, keepDims: keepdim))
}

internal func weightNorm(
    weightV: MLXArray,
    weightG: MLXArray,
    dim: Int? = nil
) -> MLXArray {
    let rank: Int = weightV.shape.count

    var axes: [Int]

    if let dim {
        var adjustedDim: Int = dim
        if dim < 0 {
            adjustedDim += rank
        }

        axes = Array(0 ..< rank)
        if adjustedDim != -1 {
            axes.removeAll { $0 == adjustedDim }
        }
    } else {
        axes = Array(0 ..< rank)
    }

    let normV: MLXArray = computeNorm(x: weightV, p: 2, dim: axes, keepdim: true)

    // Add epsilon for numerical stability
    let normalizedWeight: MLXArray = weightV / (normV + 1e-7)
    return normalizedWeight * weightG
}

/// Conv1d with weight normalization
internal class ConvWeighted: Module {
    var weightG: MLXArray
    var weightV: MLXArray
    var bias: MLXArray?

    let stride: Int
    let padding: Int
    let dilation: Int
    let outputPadding: Int
    let groups: Int

    init(
        weightG: MLXArray,
        weightV: MLXArray,
        bias: MLXArray?,
        stride: Int = 1,
        padding: Int = 1,
        dilation: Int = 1,
        outputPadding: Int = 0,
        groups: Int = 1
    ) {
        self.stride = stride
        self.padding = padding
        self.dilation = dilation
        self.outputPadding = outputPadding
        self.groups = groups

        self.weightG = weightG
        self.weightV = weightV
        self.bias = bias

        super.init()
    }

    func callAsFunction(_ x: MLXArray, conv: (MLXArray, MLXArray, Int, Int, Int, Int, StreamOrDevice) -> MLXArray) -> MLXArray {
        let weight: MLXArray = weightNorm(weightV: weightV, weightG: weightG, dim: 0)
        bias = bias?.reshaped([1, 1, -1])

        func applyConv(x: MLXArray, weightToUse: MLXArray) -> MLXArray {
            let result: MLXArray = conv(
                x,
                weightToUse,
                self.stride,
                padding,
                dilation,
                groups,
                .default
            )

            if let bias {
                return result + bias
            }
            return result
        }

        if x.shape.last == weight.shape.last || groups > 1 {
            return applyConv(x: x, weightToUse: weight)
        }
        return applyConv(x: x, weightToUse: weight.transposed())
    }

    func callAsFunction(_ x: MLXArray, conv: (MLXArray, MLXArray, Int, Int, Int, Int, Int, StreamOrDevice) -> MLXArray) -> MLXArray {
        let weight: MLXArray = weightNorm(weightV: weightV, weightG: weightG, dim: 0)
        bias = bias?.reshaped([1, 1, -1])

        func applyConv(x: MLXArray, weightToUse: MLXArray) -> MLXArray {
            let result: MLXArray = conv(
                x,
                weightToUse,
                self.stride,
                padding,
                dilation,
                outputPadding,
                groups,
                .default
            )

            if let bias {
                return result + bias
            }
            return result
        }

        if x.shape.last == weight.shape.last || groups > 1 {
            return applyConv(x: x, weightToUse: weight)
        }
        return applyConv(x: x, weightToUse: weight.transposed())
    }

    deinit {
        // MLX neural network module - no explicit cleanup needed
    }
}
