...
internal class AdainResBlk1d {
    ...
    func shortcut(_ x: MLXArray) -> MLXArray {
        var x: MLXArray = MLX.swappedAxes(x, 2, 1)
        x = upsample(x)
    ...
    func callAsFunction(x: MLXArray, s: MLXArray) -> MLXArray {
        let out: MLXArray = residual(x, s)
        return (out + shortcut(x)) / sqrt(2.0) as MLXArray
    }
