...
@MainActor
internal struct RatingRequestScreen {
    private enum Constants {
        static let maxReviewsToDisplay: Int = 7
        static let defaultAverageRating: Double = 5.0
        static let defaultTotalRatings: Int = 0
        static let noRatingsCount: Int = 0
    }
    ...
    var reviews: [Review] {
        let allReviews: [Review] = state.value?.reviews ?? []
    ...
    var averageRating: Double {
        state.value?.averageRating ?? Constants.defaultAverageRating
    }
    ...
    var totalRatings: Int {
        state.value?.totalRatings ?? Constants.defaultTotalRatings
    }

    /// Indicates whether the view should show a "no ratings" state.
    var isShowingNoRatings: Bool {
        state.value?.totalRatings == Constants.noRatingsCount && !state.isLoading
    }
    ...
    ///   - secondaryButtonAction: Optional action to perform when the user taps on
    ...
    internal init(
        ...
        primaryButtonAction: @escaping () -> Void = { /* No action */ },
    ...
    func fetchData() async {
        state = .loading
        do {
            let response: AppRatingResponse = try await appRatingProvider.fetch()
            withAnimation {
